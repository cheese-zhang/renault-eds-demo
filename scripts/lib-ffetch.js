...
async function buildElements(pressReleases, filter, createFilters, buildPressReleaseArticle, limitPerPage, cfg) {
  ...
  accordionContainer.className = 'u-padding-vertical--sm';
  const accordionDiv = document.createElement('div');
  accordionDiv.className = 'c-accordion';
  ...
  accordionGrid.className = 'o-grid';
  accordionContent.appendChild(accordionGrid);
  accordionItem.appendChild(accordionContent);

  filteredData.forEach((pressRelease) => {
    const accordionItem = document.createElement('div');
    accordionItem.className = 'c-spotlight';
    if (!relatedPressReleases && createFilters) {
      accordionItem.classList.add('u-1/4');
    } else {
      accordionItem.classList.add('u-1/3');
    }
    const pressReleaseArticle = buildPressReleaseArticle(pressRelease);
    accordionItem.appendChild(pressReleaseArticle);
    accordionGrid.appendChild(accordionItem);
  });
  accordionContainer.appendChild(accordionDiv);
  elements.list = accordionContainer;
  return elements;
}
...
export async function createList(pressReleases, filter, createFilters, buildPressReleaseArticle, limitPerPage, mainEl) {
  ...
  function attachClickListners(pagination) {
    pagination.querySelectorAll('a')
      .forEach((a) => a.addEventListener('click', reloadPaginatedList));
    return pagination;
  }
